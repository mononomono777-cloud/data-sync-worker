...
/**
 * Supabase から既存の enemy_act_history の act_id 一覧を取得
 * @returns {Set<number>} 取得済みの act_id のセット
 */
async function getExistingEnemyActIds(shortId) {
    if (!supabase || FORCE_FULL) return new Set();
    try {
        const { data, error } = await supabase
            ...
            .eq('short_id', shortId);
        if (error) {
            console.warn(`  [WARN] enemy_act_history 取得エラー: ${error.message}`);
            return new Set();
        }
        return new Set(data.map(r => r.act_id));
    } catch (e) {
        console.warn(`  [WARN] enemy_act_history 取得例外: ${e.message}`);
        return new Set();
    }
}
...
(async () => {
    ...
    try {
        ...
        for (const sid of TARGET_SIDS) {
            ...
            const allBattles = [];
            result.opponents = {}; // 初期化
            const collectedOpponentIds = new Set(); // 既に取得済みの対戦相手ID管理

            for (const matchType of MATCH_TYPES) {
                console.log(`\n  ${matchType.label} 取得中...`);
                let reachedKnown = false;
                let typeBattles = 0;
                const currentTypeNewBattles = [];
                ...
                // このマッチタイプで取得した新規バトルから対戦相手情報を取得
                if (currentTypeNewBattles.length > 0) {
                    await fetchOpponentsForBattles(page, currentTypeNewBattles, sid, result.opponents, collectedOpponentIds);
                }
            }

            result.battleLog = allBattles;
            console.log(`\n  バトルログ合計: ${allBattles.length}件`);
            console.log(`  対戦相手合計: ${Object.keys(result.opponents).length}人`);
...
/**
 * 指定されたバトルログから対戦相手を抽出し、MR履歴を取得して result.opponents に追加する
 */
async function fetchOpponentsForBattles(page, battles, mySid, opponentsMap, collectedIdsSet) {
    console.log(`\n  → 対戦相手データの取得 (${battles.length}件のバトルから)...`);
    ...
    battles.forEach(b => {
        let oppId, oppName;
        // p1_id、p2_idどちらかshort_idが取得中のIDじゃないほうが対戦相手
        if (b.p1Id === mySid) {
            oppId = b.p2Id;
            oppName = b.p2Name;
        } else {
        ...
        // 既に取得済みならスキップ
        if (collectedIdsSet.has(oppId)) return;
